async function montaGraficoUnidade(data = null){
    ...
    $.each(data, function(idx, value){ // juntar valores em um unico array
      ...
      // Data de solicitação
      if(datas.indexOf(value['Data de Solicitação']) == -1){
        datas.push(value['Data de Solicitação'])
        valor_data.push(parseFloat((value['Valor'] ? value['Valor'] : 0)))
      }
      else{
        var posic = datas.indexOf(value['Data de Solicitação'])
  ...
  var donutChart = {
    ...
    pieChart['series'] = esfera_count; pieChart['labels'] = esfera_pie;
    graficos.push(new ApexCharts(document.querySelector("#esfera"), pieChart))
    graficos[graficos.length - 1].render();
    ...
    // Tipos de servico
    // pieChart['legend']['position'] = 'right';
    pieChart['series'] = servico_count; pieChart['labels'] = tipo_servico;
    graficos.push(new ApexCharts(document.querySelector("#tipo_servico"), pieChart))
    graficos[graficos.length - 1].render();

    // Fase do projeto
    pieChart['legend']['position'] = 'bottom';
    pieChart['series'] = processo_count; pieChart['labels'] = fase_processo;
    graficos.push(new ApexCharts(document.querySelector("#fase_projeto"), pieChart))
    graficos[graficos.length - 1].render();
    ...
    var areaChart = {
      ...
      title: {
        text: 'Valor por data de solicitação',
        align: 'left'
      },
      labels: [],
      xaxis: {
    ...
    // datas.push('2021-02-03', '2021-03-03', '2021-04-03', '2021-07-03') // teste
    var valor_data = valor_data.map(a => a.toFixed(2)); // converter para duas casas decimais
    areaChart['series'][0]['data'] = valor_data; areaChart['labels'] = datas;
    graficos.push(new ApexCharts(document.querySelector("#valor_data_solicitacao"), areaChart))
    graficos[graficos.length - 1].render();

    pieChart['chart']['height'] = 220;
    // Porcentagem por polo pasta
    pieChart['series'] = [polo_ativo, polo_passivo]; pieChart['labels'] = ['Ativo', 'Passivo'];
    graficos.push(new ApexCharts(document.querySelector("#porc_polo_pasta"), pieChart))
    graficos[graficos.length - 1].render();
