...
<section class="users-list-wrapper section">
  <div class="users-list-filter">
    <div class="card-panel">
      ...
      <form role="form" id="form" action="{{ route('Painel.PesquisaPatrimonial.nucleo.finalizarempresa') }}" method="POST" role="search"  enctype="multipart/form-data">
      {{ csrf_field() }}
       ...
       <input type="hidden" name="numero" numero="numero" value="{{$numero}}">
          ...
          <label style="font-size: 11px;"  for="users-list-verified">Empresa pesquisa:</label>
            <div class="input-field">
            @if($statuspesquisa != "Conclúida")
              <select class="form-control" required id="pesquisa" name="pesquisa" style="font-size: 10px;">
            ...
            @else 
            <input style="font-size: 10px;"  name="pesquisa" id="pesquisa" type="text" value="{{$pesquisa}}" readonly> 
            @endif
            </div>
          </div>

          <div class="col s12 m6 l3 tooltipped" data-position="top" data-tooltip="Resultado da pesquisa." >
          <label style="font-size: 11px;"  for="users-list-role">Resultado da Pesquisa Empresa</label>
             <div class="input-field">
             @if($statuspesquisa != "Conclúida")
          ...
          @if($statuspesquisa != "Conclúida")
          <a class="btn btn-block waves-effect waves-light tooltipped" data-position="top" data-tooltip="Clique aqui para cadastrar uma nova informação para esta pesquisa." style="background-color: gray"  href="{{ route('Painel.PesquisaPatrimonial.empresa', ['codigo' => $codigo, 'numero' => $numero])}}"><i class="material-icons">add</i></a>
          &nbsp;
          ...
          @endif
          &nbsp;
          <a class="btn btn-block waves-effect waves-light tooltipped" data-position="top" data-tooltip="Clique aqui para gerar relatório em Excel desta pesquisa." style="background-color: gray" href="{{route('Painel.PesquisaPatrimonial.gerarexcelempresa', $codigo)}}"><img style="margin-top: 8px; width: 20px;margin-left:8px;" src="{{URL::asset('/public/imgs/icon.png')}}"/></a>
          &nbsp;
          </div>
          ...
          <table id="users-list-datatable" class="table">
            <thead>
              <tr>
                ...
                <th style="font-size: 11px;">Valor da alienação</th>
                ...
                <th style="font-size: 11px;">Editar</th>
...
<div class="modal-content">
    ...
    <div class="col s2 input-field">
     ...
     @else 
     <input style="font-size: 10px;" class="validate" value="" placeholder="Data de fundação..." readonly type="text">
     @endif
     <label style="font-size: 11px;">Data de fundação:</label>
    </div>
...
<!-- modal -->   
                ...
                <td style="font-size: 10px;"><a href="<?=route('Painel.PesquisaPatrimonial.nucleo.editarempresa', ['codigo' => $codigo, 'numero' => $numero, 'id' => $data->Id])?>"><i class="material-icons">edit</i></a></td>
...
<div id="alertastatusconcluida" name="alertastatusconcluida" class="cd-popup" role="alert" style="margin-top:130px;">
	<div class="cd-popup-container">
		<p style="font-weight: bold;">Ao selecionar a opção [Concluída], a pesquisa de [Empresa] será finalizada e enviado um e-mail e notificação ao solicitante.</p>
