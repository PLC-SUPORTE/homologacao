...
<head>
  ...
  <title>Informações de envio da solicitação de pesquisa patrimonial | Portal PL&C</title>
  <link rel="shortcut icon" href="{{ asset('/public/assets/imgs/casinha.png')}}" />
   ...
   <!-- <link rel="stylesheet" href="{{ asset('/public/materialize/css/quill.snow.css') }}"> -->
   <link rel="stylesheet" href="{{ asset('/public/materialize/css/app-sidebar.min.css') }}">
   <link rel="stylesheet" href="{{ asset('/public/materialize/css/app-email-content.css') }}">
...
<header class="page-topbar" id="header">
          ...
          <div class="nav-wrapper">
            ...
            <div class="breadcrumbs-inline pt-3 pb-1" id="breadcrumbs-wrapper" style="margin-top: -45px;">
              ...
              <ol class="breadcrumbs mb-0">
                ...
                <li class="breadcrumb-item active" style="color: black;">Atualizar informações de cobrança
                </li>
              </ol>
          </div>
 ...
 <li><a class="waves-effect waves-block waves-light profile-button" href="javascript:void(0);" data-target="profile-dropdown"><span class="avatar-status avatar-online"><img src="{{ asset('/public/AdminLTE/img/Avatar/img.png') }}" class="img-circle" alt="User Image"><i></i></span></a></li>            </ul>
  ...
  <div id="main">
    ...
    <form id="form" role="form" id="form" action="{{ route('Painel.PesquisaPatrimonial.nucleo.boleto.storeinformacoes') }}" method="POST" role="search">
      {{ csrf_field() }}


      <input type="hidden" name="clientecodigo" id="clientecodigo" value="{{$datas->ClienteCodigo}}">
      <input type="hidden" name="clienteid" id="clienteid" value="{{$datas->ClienteID}}">
    ...
    <h6 style="font-size: 20px;margin-left:-210px;">Aguarde, estamos atualizando as informações...</h6>
    </div>

   <div class="row" id="div_all">
                  
      <div
        class="content-wrapper-before " style="background-color: gray;">
      </div>
    ...
    <div class="sidebar-content">
      <div class="sidebar-header">
        <div class="sidebar-details">
          <h5 class="m-0 sidebar-title"><i class="material-icons app-header-icon text-top">mail_outline</i> E-mail de cobrança</h5>
          <div class="row valign-wrapper mt-10 pt-2">
            <div class="col s12">
              <p class="m-0 subtitle" style="font-size: 10px;">{{$datas->ClienteRazao}}</p>
              <p class="m-0 text-muted">{{$datas->ClienteEmail}}</p>   
            </div>
          </div>
        </div>
      </div>
      ...
      <div id="sidebar-list" class="sidebar-menu list-group position-relative animate fadeLeft">
        <div class="sidebar-list-padding app-sidebar sidenav" id="email-sidenav">
          <ul class="email-list display-grid">
            <li class="sidebar-title">Dados de envio</li>
            <li class="active"><a href="#!" class="text-sub"><i class="material-icons mr-2"> mail_outline </i> E-mail cliente:</a></li>
            
            <div class="input-field col m12 s12">
            <div class="form-group bmd-form-group is-filled">
            <input type="email" name="emailcliente" id="emailcliente" required value="{{$datas->ClienteEmail}}" style="font-size: 12px;">
            </div>
            </div>

            
            <li><a href="#!" class="text-sub"><i class="material-icons mr-2"> mail_outline </i> E-mail solicitante:</a></li>
            
            <div class="input-field col m12 s12">
            <div class="form-group bmd-form-group is-filled">
            <input type="email" name="emailsolicitante" id="emailsolicitante" required value="{{$EmailSolicitante}}" style="font-size: 12px;">
            </div>
            </div>
          ...
          <div class="invoice-action-btn">
            <a onClick="abreconfirmacao();" style="background-color: gray;color:white;font-size:11px;"
            ...
            <span class="responsive-text" id="btngravar">Atualizar dados</span>
            </a>
          </div>
          ...
          <div class="row">
            <div class="col s12">
              ...
              <div class="email-header">
                <div class="subject">
                  <div class="email-title" style="font-size: 16px;">[PLC][Notificação][Nota de Solicitação de Adiantamento para Pesquisa Patrimonial]</div>
                </div>

                <div class="header-action">

                  <span class="badge grey lighten-2" style="font-size: 9px;color:black;"><i class="amber-text material-icons small-icons mr-2">
                      fiber_manual_record </i>Pendente</span>
              ...
              <div class="email-content">
                <div class="list-title-area">
                  <div class="user-media">
                    <img src="{{ asset('/public/AdminLTE/img/Avatar/img.png') }}" alt="" class="circle z-depth-2 responsive-img avtar">
                    <div class="list-title">
                      <span class="name">{{$datas->ClienteRazao}}</span>
                      <span class="to-person" style="font-size: 9px;">vanessa.ferreira@plcadvogados.com.br, felipe.rocha@especialistaresultados.com.br, gumercindo.ribeiro@plcadvogados.com.br</span>
                    </div>
                  </div>
            ...
            <table class="striped responsive-table">
              ...
              @foreach($solicitacoes as $solicitacao)
                   <tr>
                   <td style="font-size: 9px">{{$solicitacao->id}} </td>
                   <td style="font-size: 9px"></td>
                   <td style="font-size: 9px"><?php echo mb_convert_case($solicitacao->PesquisadoNome, MB_CASE_TITLE, "UTF-8")?></td>
                   <td style="font-size: 9px">{{$solicitacao->Codigo}} </td>
                   <td style="font-size: 9px">{{$solicitacao->NumeroProcesso}} </td>
                   <td style="font-size: 9px"><?php echo mb_convert_case($solicitacao->ClienteRazao, MB_CASE_TITLE, "UTF-8")?></td>
                   <td style="font-size: 9px">{{ date('d/m/Y', strtotime($solicitacao->Data)) }}</td>
                   <td style="font-size: 9px"><?php echo mb_convert_case($solicitacao->SolicitanteNome, MB_CASE_TITLE, "UTF-8")?></td>
                   <td style="font-size: 9px">{{$solicitacao->TipoSolicitacao}}</td>
                   <td style="font-size: 9px">{{$solicitacao->TipoServico}}</td>
                   </tr> 
              @endforeach
              </tbody>
              ...
              <div class="email-footer">
                <h6 class="footer-title">Anexos (2)</h6>
                <div class="footer-action">
                  <div class="attachment-list">

                    <div class="attachment">
                      <img src="https://pixinvent.com/materialize-material-design-admin-template/app-assets/images/icon/pdf.png" alt="" class="responsive-img attached-image">
                      <div class="size">
                        <span class="grey-text">Boleto</span>
                      </div>
                      <div class="links">
                        <a href="{{route('Painel.PesquisaPatrimonial.nucleo.boleto.baixarboleto', $datas->CPR)}}" class="Left">
                    ...
                    <div class="attachment">
                      <img src="https://pixinvent.com/materialize-material-design-admin-template/app-assets/images/icon/pdf.png" alt="" class="responsive-img attached-image">
                      <div class="size">
                        <span class="grey-text">Nota de adiantamento</span>
                      </div>
                      <div class="links">
                        <a href="{{route('Painel.PesquisaPatrimonial.nucleo.boleto.baixarnotadebito', $datas->CPR)}}" class="Left">
                          <i class="material-icons">file_download</i>
                        </a>
                      </div>
                    </div>
...
<div id="alertaconfirmacao" name="alertaconfirmacao" class="cd-popup" role="alert">
	<div class="cd-popup-container">
		<p style="font-weight: bold;">Deseja atualizar as informações desta cobrança?</p>
		<ul class="cd-buttons">
...
<!-- <script src="{{ asset('/public/materialize/js/quill.min.js') }}"></script> -->
<script src="{{ asset('/public/materialize/js/app-email-content.js') }}"></script>
