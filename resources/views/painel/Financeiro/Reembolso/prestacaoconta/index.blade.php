...
@section('header') 
    ...
    <title>Prestação de contas index| Portal PL&C</title>
...
@media only screen and (min-width: 1170px) {
  ...
  @section('submenu')
  <li class="breadcrumb-item active" style="color: black;">Montar prestação de conta
  </li>
  @endsection
  ...
  <div class="app-wrapper">
    ...
    <div id="sidebar-list" class="row contact-sidenav ml-0 mr-0">
      ...
      <div class="col s12 m12 l12 contact-form margin-top-contact">
        ...
        <form class="col s12" role="form" id="form" action="{{ route('Painel.Financeiro.Reembolso.PrestacaoConta.relatorio') }}" method="POST" role="search" enctype="multipart/form-data" >
       {{ csrf_field() }}
       ...
       <h6 style="font-size: 20px;margin-left:-140px;">Aguarde, estamos montando o relatório..</h6>
           ...
           <div id="div_all">
            <div class="row">
              ...
              <div class="input-field col m2 s12">
              <span style="font-size: 11px;">Data de emissão ínicio:</span>
                <input style="font-size: 10px;" type="date" required name="datainicio" id="datainicio" value="{{$datahoje}}" max="{{$datahoje}}" class="validate">
              </div>

              <div class="input-field col m2 s12">
              <span style="font-size: 11px;">Data de emissão fim:</span>
                <input style="font-size: 10px;" type="date" required name="datafim" id="datafim" class="validate">
              </div>

              <div class="input-field col m2 s12">
              <span style="font-size: 11px;">Selecione a(s) fatura(s):</span>
                <select class="select2 browser-default" id="fatura" name="fatura" style="font-size:10px;">
                <option value="" selected>Selecione abaixo</option>
                </select>
              </div>
              ...
              <div class="input-field col m3 s12">
              <span style="font-size: 11px;">Selecione o banco de recebimento:</span>
                <select class="select2 browser-default" id="banco" name="banco" style="font-size:10px;">
                <option value="todos" selected>Selecione abaixo</option>
...
$('#datafim').on('change', function() {
  ...
  var _token = $('input[name="_token"]').val();
    ...
    $.ajax({
      type: 'POST',
      url:"{{ route('Painel.Financeiro.Reembolso.PrestacaoConta.buscafatura') }}",
      data:{cliente:cliente,datainicio:datainicio,datafim:datafim,_token:_token,},
      dataType: 'json',
      cache: false,
      success: function(response) {

        $('#fatura option:not(:selected)').remove();

        var selectbox = $('#fatura');
          $.each(response, function (i, d) {
              selectbox.append('<option value="' + d.Fatura+ '">' + d.Fatura + '</option>');
        });
