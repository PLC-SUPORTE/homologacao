...
@section('title') Solicitação Reembolso @endsection <!-- Titulo da pagina -->
...
@media only screen and (min-width: 1170px) {
  .cd-popup-container {
    ...
    @section('submenu')
      <li class="breadcrumb-item active" style="color: black;">Minhas solicitações de reembolso
      </li>
    @endsection
                     ...
                     <section class="invoice-list-wrapper section">
                        <div class="invoice-filter-action mr-3">
                            ...
                            <a href="#modalbuscapasta" class="waves-light btn modal-trigger" style="color: white; background-color: gray; font-size: 11px; border-radius: 50px;">
                              ...
                              <a href="{{ route('Painel.Financeiro.Reembolso.Solicitante.historico') }}" class="waves-light btn" style="color: white; background-color: gray; font-size: 11px; border-radius: 50px;">
                           ...
                           <table class="table invoice-data-table white border-radius-4 pt-1" style="font-size: 11px;">
                              ...
                              <tbody>
                                 ...
                                 <tr>
                                    ...
                                    <div id="cancelar{{$data->NumeroDebite}}" class="modal" style="width: 1207px;">
                                       <form id="formcancelar" role="form" action="{{ route('Painel.Financeiro.Reembolso.cancelar') }}" method="POST" role="create" >
                                          {{ csrf_field() }}
                                          <div class="modal-content">
                                             ...
                                             <div id="corpodiv3">
                                                ...
                                                <h5 style="font-size: 11px;">Cancelar solicitação</h5>
                                                <p style="font-size: 11px;">Confirme as informações da solicitação de reembolso para o cancelamento.
                                                <br>
                                                ...
                                                <div class="input-field col s2">
                                                   ...
                                                   <label style="font-size: 11px;" for="icon_telephone">Data:</label>
                                                </div>
                                                <div class="input-field col s2">
                                                   <input style="font-size: 10px" id="icon_telephone" readonly type="text" value="R$ <?php echo number_format($data->Valor,2,",",".") ?>" name="valor" class="validate">
                                                   <label style="font-size: 11px;" for="icon_telephone">Valor total:</label>
                                                </div>
                                                <div class="input-field col s3">
                                                   <input style="font-size: 10px" id="motivocancelamento" type="text" placeholder="Informe o motivo do cancelamento..." name="motivodescricao" class="validate">
                                                   <label style="font-size: 11px;"for="motivocancelamento">Informe o motivo do cancelamento:</label>
                                                </div>
...
{{$data->Hist}}
Solicitação de reembolso cancelada pelo(a): {{Auth::user()->name}} - {{$dataehora}} pelo motivo: </textarea>
                                                </div>
                                             </div>

                                             <div class="modal-footer" style="margin-top: 90px;">
                                                <button type="button" id="btnsubmit" onClick="cancelar();" class="modal-action  btn-flat" style="background-color: gray;color:white; margin-left: 200px; margin-top: 20px;"><i class="material-icons left">close</i>Cancelar solicitação</button>
                                             </div>
                                 ...
                                 <!--Modal editar --> 
                                    <div id="editar{{$data->NumeroDebite}}" class="modal" style="width: 1207px;height: 325px;">
                                       <form id="formeditar" role="form" action="{{ route('Painel.Financeiro.Reembolso.Solicitante.editado') }}" method="POST" role="create" enctype="multipart/form-data" >
                                          {{ csrf_field() }}
                                          <div class="modal-content">
...
Solicitação de reembolso editada pelo(a): {{Auth::user()->name}} - {{$dataehora}}</textarea>
                        ...
                        <a href="#cancelar{{$data->NumeroDebite}}" class="invoice-action-view mr-4 tooltipped modal-trigger" data-position="bottom" data-tooltip="Clique aqui para cancelar está solicitação de reembolso."><i class="material-icons">close</i></a>
                        @elseif($data->StatusID == 5)
                        <a href="#editar{{$data->NumeroDebite}}" class="invoice-action-view mr-4 tooltipped modal-trigger" data-position="bottom" data-tooltip="Clique aqui para editar está solicitação de reembolso reprovada."><i class="material-icons">edit</i></a>
                        <a href="#cancelar{{$data->NumeroDebite}}" class="invoice-action-view mr-4 tooltipped modal-trigger" data-position="bottom" data-tooltip="Clique aqui para cancelar está solicitação de reembolso."><i class="material-icons">close</i></a>
                        @endif
...
<!-- Modal Structure -->
  ...
  <form role="form" id="formbuscadados" action="{{ route('Painel.Financeiro.Reembolso.Solicitante.novasolicitacao') }}" method="POST" role="search" >
  {{ csrf_field() }}
    <center>
...
<div id="confirmacao" name="confirmacao" class="cd-popup" role="alert">
<div class="cd-popup-container">
		<p style="font-weight: bold;">Você confirma o cancelamento desta solicitação de reembolso?</p>
		<ul class="cd-buttons">
...
<div id="valorprestacaoconta" name="valorprestacaoconta" class="cd-popup" role="alert">
<div class="cd-popup-container">
<p style="font-weight: bold;">Existem valores adiantados no portal PL&C Advogados em seu nome. Favor realizar a prestação de contas para seguir com uma nova solicitação.</p>
<ul class="cd-buttons" style="width: 800px">
<li><a href="{{ route('Painel.Financeiro.AdiantamentoPrestacao.Solicitante.index') }}" onClick="nao();" style="font-weight: bold;">Realizar prestação de conta</a></li>
</ul>
<!-- <a onClick="nao();" class="cd-popup-close img-replace">Close</a> -->
</div> 
</div> 
...
function cancelar() {
  ...
  if ($('#motivocancelamento').val().length != ''){
      ...
      alert('Favor informar o motivo do cancelamento desta solicitação de reembolso.')
    }
}
</script>
