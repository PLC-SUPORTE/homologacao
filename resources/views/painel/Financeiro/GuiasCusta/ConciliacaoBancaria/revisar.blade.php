...
@media only screen and (min-width: 1170px) {
  .cd-popup-container {
    ...
    <li class="breadcrumb-item active" style="color: black;">Revisão da solicitação
    </li>
    @endsection
...
<form role="form" id="form" action="{{ route('Painel.Financeiro.GuiasCustas.ConciliacaoBancaria.revisado') }}" method="POST" role="search" enctype="multipart/form-data" >
  {{ csrf_field() }}
      ...
      <a href="#anexos{{$datas->NumeroDebite}}" class="btn-floating btn-mini waves-effect waves-light tooltipped modal-trigger"data-position="left" data-tooltip="Clique aqui para visualizar os anexos."  style="margin-left: 885px;margin-top:-10px;background-color: gray;"><i class="material-icons">attach_file</i></a>
        ...
        <div class="card-content px-36">
          ...
          <div class="invoice-item display-flex mb-1">
            ...
            <input style="font-size: 10px;"  readonly name="data" id="data" value="{{ date('d/m/Y', strtotime($datas->DataServico)) }}" type="text" class="form-control"  required="required">
            </div>
...
<ul class="collapsible collapsible-accordion">
     <li>
       ...
       <table id="scroll-dynamic" class="display">
           ...
           <tr>
           <td style="font-size: 10px;color:black">{{$contrato->Codigo}}</td>
           ...
           @endif
           <td style="font-size: 10px;color:black"><?php echo number_format($contrato->Quantidade,0,",",".") ?></td>
           <td style="font-size: 10px;color:black">R$ <?php echo number_format($contrato->Valor,2,",",".") ?></td>
           <td style="font-size: 10px;color:black">R$ <?php echo number_format($contrato->ValorCliente,2,",",".") ?></td>
           </tr>
           @endforeach
           </tbody>
           </table>
    ...
    <div class="col xl3 m4 s12" style="margin-top: -610px;">
      <div class="card invoice-action-wrapper mb-10">
        <div class="card-content">

        <p>
        <label>
        <input style="font-size: 11px;" name="statusescolhido" class="with-gap" value="baixarsolicitacao" checked type="radio" checked onClick="baixarsolicitacao();"/>
        <span>Baixar solicitação</span>
        </label> 
        </p>
        ...
        <div class="row" style="font-size:10px" id="statusdebite">
        <div class="input-field col m12 s12">
        <span>Está solicitação é:</span>
        <div class="form-group bmd-form-group is-filled">
        <select class="browser-default" required="required" style="width: 100%;font-size:10px;" id="statusdebite" name="statusdebite"  data-toggle="tooltip" data-placement="top" title="Selecione se esta solicitação é reembonsável ou não cobrável.">
        ...
        <!--Baixar solicitação --> 
        <div class="row" style="font-size:10px" id="baixarsolicitacaodiv">

        <div class="input-field col m12 s12">
        <span>Gerar CPR pro cliente:</span>
        <div class="form-group bmd-form-group is-filled">
        <select class="browser-default" style="width: 100%;font-size:10px;" id="gerarcpr" name="gerarcpr"  data-toggle="tooltip" data-placement="top" title="Selecione se deseja gerar CPR pro cliente neste momento.">
        ...
        <div class="input-field col m12 s12" id="datavencimento">
        <span style="font-size: 11px;">Data de vencimento:</span>
        <div class="form-group bmd-form-group is-filled">
        <input style="font-size: 10px;" type="date" name="datavencimento" id="datavencimento" value="{{$datapagamento}}">
        </div>
        </div>
        ...
        <div class="form-group bmd-form-group is-filled">
        <select class="browser-default" style="width: 100%;font-size:10px;" id="tipodocumento" name="tipodocumento"  data-toggle="tooltip" data-placement="top" title="Selecione o tipo de documento.">
        ...
        <div class="form-group bmd-form-group is-filled">
        <select class="browser-default" style="width: 100%;font-size:10px;" id="tipolan" name="tipolan"  data-toggle="tooltip" data-placement="top" title="Selecione o tipo de lançamento.">
        ...
        <div class="input-field col m12 s12">
        <span>Banco de saída:</span>
        <div class="form-group bmd-form-group is-filled">
        <select class="browser-default" style="width: 100%;font-size:10px;" id="portador" name="portador"  data-toggle="tooltip" data-placement="top" title="Selecione o banco de saída.">
        ...
        <div class="input-field col m12 s12">
        <span style="font-size: 11px;">Data da conciliação:</span>
        <div class="form-group bmd-form-group is-filled">
        <input style="font-size: 10px;" type="date" name="dataconciliacao" id="dataconciliacao" value="{{$datapagamento}}">
        </div>
        </div>

        <div class="input-field col m12 s12">
        <span style="font-size: 11px;">Data da baixa:</span>
        <div class="form-group bmd-form-group is-filled">
        <input style="font-size: 10px;" type="date" name="databaixa" id="databaixa" value="{{$datapagamento}}">
        </div>
        </div>

        <div class="input-field col m12 s12">
        <span style="font-size: 11px;">Comprovante de pagamento</span>
        <input style="font-size: 10px;" type="file" name="select_file[]" multiple id="input-file-now" class="dropify" accept=".pdf" data-default-file="" />
        </div>


        </div>
        <!--Fim Baixar solicitação --> 
        ...
        <div class="invoice-action-btn">
            <a onClick="abreconfirmacao();" style="background-color: gray;color:white;font-size:11px;" class="btn waves-effect waves-light display-flex align-items-center justify-content-center">
              ...
              <span class="responsive-text" id="btngravar">Baixar solicitação</span>
            </a>
        </div> 
...
<div id="baixar" class="cd-popup" role="alert">
	<div class="cd-popup-container">
		<p style="font-weight: bold;">Deseja baixar a solicitação de pagamento de guia de custa?</p>
		<ul class="cd-buttons">
...
{{$datas->Hist}}
Número da solicitação: {{$datas->NumeroDebite}}. Solicitação de pagamento de guia de custa revisada pelo(a): {{Auth::user()->name}} pela equipe do financeiro - {{$dataehora}}</textarea>
              </div> 

              <div class="input-field col s12" style="display: none">
                    <textarea id="histbaixado" rows="4" type="text" name="histbaixado" readonly="" class="form-control" style="text-align: left;margin: 0;" placeholder="Hist debite">
{{$datas->Hist}}
Número da solicitação: {{$datas->NumeroDebite}}. Solicitação de pagamento de guia de custa revisada pelo(a): {{Auth::user()->name}} pela equipe do financeiro - {{$dataehora}}</textarea>
Número da solicitação: {{$datas->NumeroDebite}}. Solicitação de pagamento de guia de custa baixada pelo(a): {{Auth::user()->name}} pela equipe do financeiro - {{$dataehora}}</textarea>
              </div> 
...
{{$datas->Hist}}
Número da solicitação: {{$datas->NumeroDebite}}. Solicitação de pagamento de guia de custa reprovada pelo(a): {{Auth::user()->name}} pela equipe do financeiro - {{$dataehora}} com o motivo: </textarea>
              </div> 
...
{{$datas->Hist}}
Número da solicitação: {{$datas->NumeroDebite}}. Solicitação de pagamento de guia de custa cancelada pelo(a): {{Auth::user()->name}} pela equipe do financeiro - {{$dataehora}} com o motivo: </textarea>
              </div> 
...
function abreconfirmacao() {
  ...
  if(status == "baixarsolicitacao") {
    $('#baixar').addClass('is-visible');
...
<script>
function baixarsolicitacao() {
    document.getElementById("justificativadiv").style.display = "none";    
    document.getElementById("baixarsolicitacaodiv").style.display = "";      
    document.getElementById("statusdebite").style.display = "";                                                                     
  }    
</script>
...
function abrirjustificativa() {
    ...
    document.getElementById("baixarsolicitacaodiv").style.display = "none";   
    document.getElementById("statusdebite").style.display = "none";                                            
  }    
</script>
...
<script>
  ...
  //Altera o label do botão conforme a opção marcada
   if(status == "baixarsolicitacao") {
    $('#btngravar').html('Baixar solicitação');
  }
    else if(status == "reprovar") {
...
<script>
$('#gerarcpr').blur(function(){

  var status = $('#gerarcpr').val();

  //Altera o label do botão conforme a opção marcada
   if(status == "SIM") {
    document.getElementById("datavencimento").style.display = "";                                                                     
  } else {
