...
<section class="invoice-list-wrapper section">
  ...
  <div class="responsive-table">
    ...
    <!--Inicio Modal Anexar Documentação --> 
    <div id="anexar{{$data->id}}" class="modal" style="width: 1207px;">
    <form id="form" role="form" onsubmit="btnsubmit.disabled = true; return true;"  action="{{ route('Painel.DPRH.Desligamento.RH.store') }}" method="POST" role="create" enctype="multipart/form-data" >
    {{ csrf_field() }}
  ...
  <button type="button" class="btn waves-effect mr-sm-1 mr-2 modal-close red" style="margin-top: -26px; margin-left: 1080px;position: fixed;"><i class="material-icons">close</i></button>
      ...
      <h6>[Workflow etapa 3 - Anexar documentação de desligamento]</h6>
      <p style="font-size: 11px;">Favor anexar documentação para finalizar o desligamento.</p>
...
<!--Fim Modal Anexar documentação --> 
          ...
          @if($data->StatusID == 4)
          <a href="#anexar{{$data->id}}" class="invoice-action-view mr-4 tooltipped modal-trigger" data-position="left" data-tooltip="Clique aqui para visualizar os anexos desta solicitação de contratação."><i class="material-icons">attachment</i></a> 
          @endif
          <!--Fim --> 
...
<div id="loadingdiv3" style="display:none">
  <div style="height: 50px;margin-top: calc(50vh - 150px);margin-left: calc(50vw - 600px);width: 180px;">
    ...
    <div id="corpodiv3">
    <div class="modal-content">
      <center><p style="font-size: 18px;">Deseja confirmar o anexo da documentação de desligamento?</p></center>
    </div>
...
<script>
  function abremodalconfirmando() {
    ...
    var termodevolucaoequipamento = $('#termodevolucaoequipamento').val();
    var rescisao = $('#rescisao').val();

    if ($('#termodevolucaoequipamento').val().length != 0 && $('#rescisao').val().length != 0){
      ...
      alert('Favor realizar o anexo dos campos.')
