...
<section class="invoice-list-wrapper section">
  ...
  <a href="{{ route('Painel.DPRH.Desligamento.Superintendente.historico') }}" class="waves-effect waves-light btn tooltipped border-round" data-position="left" data-tooltip="Clique aqui para visualizar as solicitações já finalizadas ou canceladas." style="background-color: gray;font-size:11px;"><i class="material-icons">list</i>Histórico</a>
  </div>
...
<div id="glosar{{$data->id}}" class="modal" style="width: 1207px;">
    <form id="form2" role="form" onsubmit="btnsubmit.disabled = true; return true;"  action="{{ route('Painel.DPRH.Desligamento.Superintendente.solicitacaoglosada') }}" method="POST" role="create" enctype="multipart/form-data" >
    {{ csrf_field() }}
...
<div id="modal" class="modal" style="width: 1207px;">
    <form id="form" role="form" onsubmit="btnsubmit.disabled = true; return true;"  action="{{ route('Painel.DPRH.Desligamento.Superintendente.store') }}" method="POST" role="create" enctype="multipart/form-data" >
    {{ csrf_field() }}
