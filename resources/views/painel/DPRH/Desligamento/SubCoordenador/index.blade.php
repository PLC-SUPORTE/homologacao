...
<section class="invoice-list-wrapper section">
  ...
  <a href="{{ route('Painel.DPRH.Desligamento.SubCoordenador.historico') }}" class="waves-effect waves-light btn tooltipped border-round" data-position="left" data-tooltip="Clique aqui para visualizar as solicitações já finalizadas ou canceladas." style="background-color: gray;font-size:11px;"><i class="material-icons">list</i>Histórico</a>
  </div>
...
<div id="glosar{{$data->id}}" class="modal" style="width: 1207px;">
    <form id="form2" role="form" onsubmit="btnsubmit.disabled = true; return true;"  action="{{ route('Painel.DPRH.Desligamento.SubCoordenador.solicitacaoglosada') }}" method="POST" role="create" enctype="multipart/form-data" >
    {{ csrf_field() }}
...
<!--Fim Modal Liberar Solicitação --> 
          ...
          @if($data->StatusID == 1)
          <a href="#glosar{{$data->id}}" class="invoice-action-view mr-4 tooltipped modal-trigger" data-position="left" data-tooltip="Clique aqui caso deseje glosar está solicitação de desligamento."><i class="material-icons">notification_important</i></a> 
          @elseif($data->StatusID == 3)
          <a href="#liberar{{$data->id}}" class="invoice-action-view mr-4 tooltipped modal-trigger" data-position="left" data-tooltip="Clique aqui para liberar está solicitação de desligamento."><i class="material-icons">touch_app</i></a> 
          @endif
          <!--Fim --> 
...
<div id="modal" class="modal" style="width: 1207px;">
    <form id="form" role="form" onsubmit="btnsubmit.disabled = true; return true;"  action="{{ route('Painel.DPRH.Desligamento.SubCoordenador.store') }}" method="POST" role="create" enctype="multipart/form-data" >
    {{ csrf_field() }}
