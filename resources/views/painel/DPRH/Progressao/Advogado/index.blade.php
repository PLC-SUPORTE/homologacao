...
<div id="modal" class="modal" style="width: 1207px;">
    <form id="form" role="form" onsubmit="btnsubmit.disabled = true; return true;"  action="{{ route('Painel.DPRH.Progressao.Advogado.store') }}" method="POST" role="create" enctype="multipart/form-data" >
    {{ csrf_field() }}
  ...
  <button type="button" class="btn waves-effect mr-sm-1 mr-2 modal-close red" style="margin-top: -26px; margin-left: 1080px;position: fixed;"><i class="material-icons">close</i></button>
      ...
      <h6>[Workflow etapa 1 - Cadastro de solicitação de progressão]</h6>
      <p style="font-size: 11px;">Favor informar os dados para cadastro de um nova progressão.</p>
      ...
      <br>
        ...
        <div class="input-field col m2 s12">
            ...
            <select style="font-size: 10px;" class="select2 browser-default" id="setor" name="setor"  data-toggle="tooltip" data-placement="top" title="Selecione o setor de custo." required="required">
                <option value="{{$dados->SetorCodigo}}" selected>{{$dados->SetorCodigo}} - {{$dados->SetorDescricao}}</option>
            </select>
        </div>

        <div class="input-field col m2 s12">
            <span style="font-size: 11px;">Distribuição mensal:</span>
            <input style="font-size: 10px;" id="icon_telephone" type="text" value="<?php echo number_format($dados->Salario,2,",",".") ?>" name="salario" class="validate" maxlength="8" pattern="(?:\.|,|[0-9])*" placeholder="Valor(R$)" onKeyPress="return(moeda2(this,'.',',',event))">
        </div>
