...
<section class="invoice-list-wrapper section">
  ...
  <div class="responsive-table">
    ...
    <form id="form" role="form" onsubmit="btnsubmit.disabled = true; return true;"  action="{{ route('Painel.DPRH.Ferias.RH.solicitacaorevisada') }}" method="POST" role="create" enctype="multipart/form-data" >
    {{ csrf_field() }}

    <input type="hidden" name="id" id="id" value="{{$data->id}}">
    <input type="hidden" name="solicitante_id" id="solicitante_id" value="{{$data->solicitante_id}}">
    <input type="hidden" name="solicitante_email" id="solicitante_email" value="{{$data->solicitante_email}}">
    <input type="hidden" name="solicitante_cpf" id="solicitante_cpf" value="{{$data->solicitante_cpf}}">
  ...
  <button type="button" class="btn waves-effect mr-sm-1 mr-2 modal-close red" style="margin-top: -26px; margin-left: 1080px;position: fixed;"><i class="material-icons">close</i></button>
      ...
      <h6>[Workflow etapa 6 - Anexar documentação desta solicitação de férias]</h6>
      <p style="font-size: 11px;">Favor realizar o anexa da documentação desta solicitação de férias.</p>
    ...
    <div class="row">
       ...
       <div class="input-field col m2 s12">
            <span style="font-size: 11px;">Solicitante:</span>
            <input style="font-size: 10px;"  readonly name="solicitante" id="solicitante" value="{{$data->SolicitanteNome}}"  required="required">
        </div>

        <div class="input-field col m2 s12">
            <span style="font-size: 11px;">Setor de custo:</span>
            <input style="font-size: 10px;"  readonly name="setor" id="setor" value="{{$data->Setor}}"  required="required">
        </div>

        <div class="input-field col m2 s12">
        <span style="font-size: 11px;">Data ínicio:</span>
        <input style="font-size: 10px;"  readonly name="datainicio" id="datainicio" value="{{ date('d/m/Y', strtotime($data->DataFeriasInicio)) }}" type="text"  class="form-control" data-toggle="tooltip" data-placement="top" title="Informe a data prevista para ínicio." required="required">
        </div>


        <div class="input-field col m2 s12">
        <span style="font-size: 11px;">Data saída:</span>
        <input style="font-size: 10px;"  readonly name="datasaida" id="datasaida" value="{{ date('d/m/Y', strtotime($data->DataFeriasFim)) }}" type="text"  class="form-control" data-toggle="tooltip" data-placement="top" title="Informe a data prevista para volta." required="required">
        </div>
       ...
       <div class="row">
        ...
        <span style="font-size: 11px;">Clique aqui para anexar a documentação:</span>
        <input style="font-size: 10px;"  name="documento" id="documento" value="" type="file" required>
       </div>
       </div>


      <div class="modal-footer" style="margin-top: 0px;">
      <button type="button" id="btnsubmit" onClick="abremodalconfirmando();"  class="modal-action waves-effect waves-green btn-flat" style="background-color: gray;color:white;font-size: 11px;"><i class="material-icons left">save</i>Salvar documentação</button>
      </div>
...
<!--Fim Modal Anexar documentação--> 
          ...
          <td style="font-size: 10px">{{ date('d/m/Y', strtotime($data->DataFeriasInicio)) }}</td>
          <td style="font-size: 10px">{{ date('d/m/Y', strtotime($data->DataFeriasFim)) }}</td>         
          ...
          @if($data->StatusID == 5)
          <a href="#modalrevisar{{$data->id}}" class="invoice-action-view mr-4 tooltipped modal-trigger" data-position="bottom" data-tooltip="Clique aqui para anexar o documento para está solicitação de férias."><i class="material-icons">attach_file</i></a>
          @endif
...
<script>
  function abremodalconfirmando() {
    ...
    var documentacao = $('#documento').val();
    ...
    if ($('#documento').val().length != 0){
        ...
        alert('Favor realizar o anexo da documentação!');
    }
