...
<html class="loading" lang="pt-br" data-textdirection="ltr">
  <head>
    ...
    <title>DPRH Férias revisar solicitação | Portal PL&C</title>
          ...
          <div class="nav-wrapper">
            ...
            <div class="breadcrumbs-inline pt-3 pb-1" id="breadcrumbs-wrapper" style="margin-top: -45px;">
              ...
              <ol class="breadcrumbs mb-0">
                ...
                <li class="breadcrumb-item"><a href="{{ route('Painel.DPRH.Ferias.Superintendente.index') }}">Solicitações em andamento</a>
                </li>
...
<div id="app-calendar">
  <div class="row">
   ...
   <form id="form" role="form" action="{{ route('Painel.DPRH.Ferias.Superintendente.solicitacaorevisada') }}" method="POST" role="create" enctype="multipart/form-data" >
    {{ csrf_field() }}

    <input type="hidden" name="id" id="id" value="{{$data->id}}">
    <input type="hidden" name="solicitante_email" value="{{$data->SolicitanteEmail}}">
    <input type="hidden" name="solicitante_id" value="{{$data->SolicitanteID}}">
      ...
      <h6>[Workflow etapa 4 - Revisão da solicitação de férias]</h6>
      <p style="font-size: 11px;">Favor realizar a revisão da solicitação de férias.</p>
      ...
      <div class="row">
        ...
        <div class="input-field col s3">
          <input readonly id="solicitantenome" type="text" name="solicitantenome" value="{{ $data->SolicitanteNome }}" class="validate">
          <label for="solicitantenome">Solicitante:</label>
        </div>

        <div class="input-field col s2">
          <input id="setor" readonly type="text" name="setor"  value="{{ $data->Setor}}" class="validate">
          <label for="setor">Setor:</label>
        </div>

        <div class="input-field col s2">
          <input id="datainicio" type="date" value="{{ date('Y-m-d', strtotime($data->DataFeriasInicio)) }}" name="datainicio" min="{{$datahoje}}" class="validate">
          <label for="datainicio">Data ínicio:</label>
        </div>

        <div class="input-field col s2">
        <input id="datafim" type="date" value="{{ date('Y-m-d', strtotime($data->DataFeriasFim)) }}" name="datafim" min="{{$datahoje}}" class="validate">
    ...
    <div class="modal-footer" style="margin-top: 40px;margin-left: 20px;">
      <button type="submit" id="btnsubmit" name="acao" value="2" class="modal-action waves-effect waves-green btn-flat" style="background-color: gray;color:white;"><i class="material-icons left">close</i>Glosar</button>
      <button type="button" id="btnsubmit" name="acao" onClick="abremodalconfirmando();" value="1" class="modal-action waves-effect waves-green btn-flat" style="background-color: gray;color:white;"><i class="material-icons left">check</i>Aprovar solicitação</button>
...
<div id="loadingdiv3" style="display:none">
  <div style="height: 50px;margin-top: calc(50vh - 150px);margin-left: calc(50vw - 600px);width: 180px;">
    ...
    <div class="modal-content">
      <center><p style="font-size: 18px;">Deseja confirmar a revisão da solicitação de férias?</p></center>
    </div>
...
$(document).ready(function() {
    ...
    $('#basic-calendar').fullCalendar({
      ...
      events: [
        ...
        @foreach ($datas as $data)
        {
          title: '{{$data->SolicitanteNome}}',
          id: {{$data->id}},
          start: '{{ date("Y-m-d H:i:s", strtotime($data->DataFeriasInicio)) }}',
          end: '{{ date("Y-m-d H:i:s", strtotime($data->DataFeriasFim)) }}',
          color: 'gray',
      ...
      eventClick: function (calEvent, jsEvent, view) {
                ...
                var etime = calEvent.end.format('DD/MM/YYYY');
