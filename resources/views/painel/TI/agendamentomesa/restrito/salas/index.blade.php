...
<html class="loading" lang="pt-br" data-textdirection="ltr">
  <head>
    ...
    <title>Listagem salas | Portal PL&C</title>
...
<section class="invoice-list-wrapper section">
  ...
  <div class="invoice-filter-action mr-3">
    <a href="{{ route('Painel.TI.agendamentomesa.restrito.salas.novasala') }}" class="btn waves-effect waves-light invoice-export border-round z-depth-4" style="background-color: gray">
      <i class="material-icons">add</i>
      <span class="hide-on-small-only">Nova sala</span>
    </a>
  </div>
    ...
    <table class="table invoice-data-table white border-radius-4 pt-1">
      ...
      @foreach($datas as $data)
        <tr>
          ...
          <td>{{$data->TipoDescricao}}</td>
          <td>{{$data->Andar}}</td>
          <td>{{$data->Unidade}}</td>
          <td>{{$data->UnidadeDescricao}}</td>
          @if($data->Status == "A")
          <td><span class="bullet green"></span><small>Ativa</small></td>
          @else 
          <td><span class="bullet red"></span><small>Desativada</small></td>
          @endif
          <td>
            <div class="invoice-action">
              @if($data->Tipo == 1)
              <a href="{{ route('Painel.TI.agendamentomesa.restrito.mesas.index', $data->id) }}" class="invoice-action-view mr-4">
                <i class="material-icons">remove_red_eye</i>
              </a>
              @else 
              <a href="{{ route('Painel.TI.agendamentomesa.restrito.mesas.agendareuniao', $data->id) }}" class="invoice-action-view mr-4"><i class="material-icons">calendar_today</i></a>
              @endif
            </div>
          </td>
