                  ...
                  <div class="card-content">
                     ...
                     <div id="andar8" class="col s12">
                        ...
                        <div class="row" style="margin-top: 80px; margin-left: 250px;">
                          ...
                          <div class="linha-entrada tooltipped"  data-position="left" data-tooltip="Entrada"></div>
                         
                          <!--Fileira A --> 
                          <!--Cadeiras A Esquerda --> 
                           <div class="col l1">
                              <p style="font-size: 16px; margin-top: -60px; margin-left: 26px; color: black;"><b>A</b></p>
                             <div class="container" style="margin-left: -30px; margin-top: 37px;">
                             @foreach($datas as $data)
                              @if($data->corredor == "A1E")
                              @if($data->agendamento == null)
                                @if($data->identificacao == $cor)
                               <a onClick="pegadados({{$data->id}});" class="material-icons iconSeatEsq tooltipped" data-position="left" data-tooltip="{{$data->descricao}} disponível. " 
                                ...
                                event_seat
                                 ...
                                 @endif 
                                <!--Fim validação da cor -->  
                                @elseif(date('Y-m-d', strtotime($data->datainicio)) != $datainicio)
                                   @if($data->identificacao == $cor)
                                 <a onClick="pegadados({{$data->id}});" class="material-icons iconSeatEsq tooltipped" data-position="left" data-tooltip="{{$data->descricao}} disponível. " 
                                style="color:#28a745; background-color: #EFEDED; border-color: #EFEDED; border-radius: 10px;" id="{{$data->id}}" data-id="{{$data->descricao}}" 
                                data-andar="{{$data->andar}}" data-sala="{{$data->sala}}" data-corredor="A" name="mesa{{$data->id}}" value="{{$data->id}}">
                                event_seat
                                   ...
                                   @endif
                                @elseif(date('Y-m-d', strtotime($data->datainicio)) == $datainicio &&  $data->datainicio < $startTime && $data->datafim < $endTime && $data->identificacao == $cor)
                                 <a onClick="pegadados({{$data->id}});" class="material-icons iconSeatEsq tooltipped" data-position="left" data-tooltip="{{$data->descricao}} disponível. " 
                                style="color:#28a745; background-color: #EFEDED; border-color: #EFEDED; border-radius: 10px;" id="{{$data->id}}" data-id="{{$data->descricao}}" 
                                data-andar="{{$data->andar}}" data-sala="{{$data->sala}}" data-corredor="A" name="mesa{{$data->id}}" value="{{$data->id}}">
                                event_seat
                           ...
                           <!--Fim Cadeiras A Esquerda --> 

                        <!--Cadeiras A Direita -->
                        <div class="col l1">
                           <div class="col l1">
                              <img rel="shortcut icon" id="mesa6spo" src="{{ asset('/public/assets/imgs/mesabaixo2.png')}}"/>
                              <div class="container" style="margin-left: -8px; margin-top: -112px;">
                              @foreach($datas as $data)
                              @if($data->corredor == "A1D")
                              @if($data->agendamento == null)
                               <!--Verifica se a cor é a do dia, se não fica indisponivel(Cinza) -->
                                @if($data->identificacao == $cor)
                        ...
                        <!--Fileira B Superior --> 
                          ...
                          <div class="col l1">
                           <p style="font-size: 16px; margin-top: -60px; margin-left: 120px; color: black;"><b>B</b></p>
                            <div class="container" style="margin-left: 55px; margin-top: 74px;">
                           @foreach($datas as $data)
                        ...
                        <!--Fim Cadeiras B Esquerda --> 

                       <!--Cadeira individual da Fileira B --> 
                        <div class="col l1">
                          <div class="container" style="margin-left: 57px; margin-top: -30px;">
                           @foreach($datas as $data)
                              @if($data->corredor == "B2")
                              @if($data->agendamento == null)
                                @if($data->identificacao == $cor)
                               <a onClick="pegadados({{$data->id}});" class="material-icons tooltipped" data-position="left" data-tooltip="{{$data->descricao}} disponível. " 
                                style="color:#28a745; background-color: #EFEDED; border-color: #EFEDED; border-radius: 10px;" id="{{$data->id}}" data-id="{{$data->descricao}}" 
                                data-andar="{{$data->andar}}" data-sala="{{$data->sala}}" data-corredor="B" name="mesa{{$data->id}}" value="{{$data->id}}">
                                event_seat
                                 </a>
                                 @else 
                                 <span class="material-icons tooltipped" data-position="left" data-tooltip="Estação de trabalho indisponível para agendamento na data de hoje. " style="color:#D4D4D4;"background-color: #EFEDED; border-color: #EFEDED; border-radius: 10px;">
                                 event_seat
                                 </span>
                                 @endif 
                                 @elseif(date('Y-m-d H:i', strtotime($data->datainicio)) < $datahoje && date('Y-m-d H:i', strtotime($data->datafim)) < $datahoje && $data->identificacao == $cor)
                                 <a onClick="pegadados({{$data->id}});" class="material-icons iconSeatEsq tooltipped" data-position="left" data-tooltip="{{$data->descricao}} disponível. " 
                                style="color:#28a745; background-color: #EFEDED; border-color: #EFEDED; border-radius: 10px;" id="{{$data->id}}" data-id="{{$data->descricao}}" 
                                data-andar="{{$data->andar}}" data-sala="{{$data->sala}}" data-corredor="B" name="mesa{{$data->id}}" value="{{$data->id}}">
                                event_seat
                               ...
                               @else  
                              <span class="material-icons tooltipped" data-position="left" data-tooltip="Estação de trabalho agendada para o(a) sócio(a): " style="color:#dc3545; background-color: #EFEDED; border-color: #EFEDED; border-radius: 10px;">
                              event_seat
                              </span>
                              @endif
                              @endif
                      ...
                      <!--Fim Cadeira Individual da Fileira B --> 
                      
                          <!--Cadeiras da Direita --> 
                          <div class="col l1">
                            <img rel="shortcut icon" style="transform: rotate(180deg);" src="{{ asset('/public/assets/imgs/rjo5.png')}}"/>
                           <div class="container" style="margin-left: 60px; margin-top: -75px;">
                            @foreach($datas as $data)
