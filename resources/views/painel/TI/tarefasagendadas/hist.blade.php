@extends('Painel.TI.Layouts.index')

@section('content')
    <div class="content-wrapper">
        <section class="content-header">
            <h1>
                [Listagem] -
                <small>[Acompanhamento de tarefa no Portal PL&C]</small>
            </h1>
            <ol class="breadcrumb">
                <li><a href="{{ route('Home.Principal.Show') }}"><i class="fa fa-dashboard"></i> Home</a></li>
                <li class="active">Acompanhamento tarefa</li>
            </ol>
        </section>
        <section class="content">
            @include('flash::message')
            <div class="row">
                <div class="col-xs-12">
                  <div class="float-right">
                      <small>
                          <a href="" class="delete" target="_blank"><span class="btn btn-success fa fa-file-excel-o"> Exportar Dados</span></a>
                      </small>
                  </div>
                    <div class="box ">
                    <div class="box-body">
                        <div class="table-responsive">
                        <table id="example1" class="table table-bordered table-striped" style="font-size: 12px;">
                                <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Modúlo</th>
                                    <th>Frequência</th>
                                    <th>Data executado</th>
                                    <th>Proximo Horário</th>
                                    <th>Status</th>
                                </tr>
                                </thead>
                                <tbody>
                                
                                @foreach($datas as $data)
                                    <tr>
                                        <td>{{ $data->Id }}</td>                  
                                        <td>{{ $data->Modulo}} </td>
                                        <td>{{ $data->Frequencia}} </td>
                                        <td>{{ date('d/m/Y H:i:s', strtotime($data->Data)) }}</td>
                                        <td>{{ date('d/m/Y H:i:s', strtotime($data->DataProximoHorario)) }}</td>
                                        <td>{{ $data->Observacao}}</td>
                                    </tr>
                                @endforeach

                                </tbody>
                            </table>
                        </div>
                    </div>

                </div>

            </div>
        </section>
    </div>
@endsection


