...
<html class="loading" lang="pt-br" data-textdirection="ltr">
  <head>
    ...
    <title>Escritório - Minhas solicitações | Portal PL&C</title>
...
<?php
      ...
      { 
       $output = '';
       $query = "SELECT * FROM dbo.Escritorio_Solicitacoes_Produtos ORDER BY descricao ASC";
...
<div class="invoice-filter-action mr-3">
  ...
  <a  href="{{ route('Painel.Escritorio.Solicitacoes.Advogado.historico') }}" class="waves-effect waves-light btn tooltipped border-round" data-position="left" data-tooltip="Clique aqui para visualizar as solicitações já finalizadas ou canceladas." style="background-color: gray;font-size:11px;"><i"><i class="material-icons">list</i>Histórico</a>
  </div>
  ...
  <div id="cancelar{{$data->Id}}" class="modal"  style="width: 600px;">
     <form id="formcancelar" role="form" action="{{ route('Painel.Escritorio.Solicitacoes.Advogado.solicitacacancelada') }}" method="POST" role="create"  enctype="multipart/form-data">
     {{ csrf_field() }}
     ...
     <div style="height: 50px;margin-top: calc(50vh - 150px);margin-left: calc(50vw - 600px);width: 180px;">
              <img style="width: 100px;margin-left:180px;margin-top:-70px;" src="{{URL::asset('/public/imgs/loading.gif')}}"/>
</div>
    </div>

    <div id="corpodiv4">
    <div class="modal-content">
      <center><p style="font-size: 18px;">Deseja confirmar o cancelamento desta solicitação de compra?</p></center>
    </div>
    <div class="modal-footer">
      ...
      <a  class="modal-action  waves-effect waves-green btn-flat " style="background-color: green;color:white;font-size:11px;" onClick="cancelasim();"><i class="material-icons left">check</i>Sim</a>
    </div>
    </div>
    </form>
  </div>
            ...
            @if($data->StatusID == 1)
            <a href="#cancelar{{$data->Id}}" class="invoice-action-view mr-4 tooltipped modal-trigger" data-position="bottom" data-tooltip="Clique aqui para cancelar está solicitação de compra."><i class="material-icons">close</i></a>
            @endif
      ...
      <div id="modal" class="modal" style="width: 1207px;">
         <form id="form" role="form" action="{{ route('Painel.Escritorio.Solicitacoes.Advogado.store') }}" method="POST" role="create"  enctype="multipart/form-data">
          {{ csrf_field() }}
    <div class="modal-content">
  <div id="corpodiv2">
          ...
          <div class="table-repsonsive">
            <table class="table table-bordered" id="item_table" style="font-size: 10px;">
                <tr>
                    ...
                    <th>
                    <button type="button" name="add" style="width: 170px;background-color:gray;"
                            class="waves-effect waves-light btn add tooltipped border-round"
                            data-position="left" data-tooltip="Clique aqui para adicionar um novo produto para está solicitação.">
                    ...
                    <td>
                    <select class="browser-default" id="destino_selected" name="destino_selected[]" required>
                    <option value="" selected></option>
                    @foreach ($areas as $area)
                    <option value="{{$area->id}}" id="{{$area->id}}">{{$area->descricao}}</option>
                    @endforeach
                    </select>
                    </td>

                    <td>
                    <select class="browser-default" id="produtos" name="produtos[]" required>
                    <option selected="selected" value=""></option>
                    </select>
                    </td>

                    <td>
                    <input style="font-size: 10px;" id="quantidade" type="number"  name="quantidade[]" class="validate" required>
                    </td>
            ...
            <div class="input-field col m12 s12">
                  ...
                  <textarea style="font-size: 10px; height: 45px;"  id="observacao" name="observacao"></textarea>
              </div>
             ...
             <div class="modal-footer">
               <button type="button" id="button_save"  onClick="abremodalconfirmando();" 
                class="modal-action waves-effect btn-flat border-round"
                style="background-color: gray;color:white;font-size: 11px; margin-left: 50px;">
                <i class="material-icons left">save</i>Salvar nova solicitação</button>
             </div>
    ...
    <div class="modal-footer">
      ...
      <script>
            ...
            $('#destino_selected').on('change', function() {
               ...
               var id_destino = $('#destino_selected').val();

               $.ajax({
                  url:"{{ route('Painel.Escritorio.Solicitacoes.Advogado.buscaprodutos') }}",
                  type: 'POST',
                  data:{_token: _token, id_destino: id_destino},
...
<script>
         $(document).ready(function(){
          ...
          $(document).on('click', '.add', function(){
           var html = '';
           html += '<tr>';
           html += '<td><select style="font-size: 10px;" class="select2 browser-default" id="setor" name="setor[]"  data-toggle="tooltip" data-placement="top" title="Selecione o setor de custo." required="required">@foreach($setores as $setor)<option value="{{$setor->Codigo}}">{{$setor->Codigo}} - {{$setor->Descricao}}</option>@endforeach</select></td>';
           html += '<td><select class="browser-default" id="destino_selected" name="destino_selected[]" required><option value="" selected></option>@foreach ($areas as $area)<option value="{{$area->id}}" id="{{$area->id}}">{{$area->descricao}}</option>@endforeach</select></td>'; 
           html += '<td><select class="browser-default" id="produtos" name="produtos[]" required><option selected="selected" value=""></option><?php echo fill_unit_select_box($connect); ?></select></td>';
           html += '<td><input style="font-size: 10px;" id="quantidade" type="number"  name="quantidade[]" class="validate" required></td>';
